...
permalink: 3_3.html
summary: "No, you're not crazy; MS's solution IS pretty ridiculous (but still viable)."
---


### TL/DR

Microsoft provides optional commercial data protection (CDP) functionality for the generative-AI services Copilot and Bing-chat. The CDP function can only be *enforced* via policy if users are signed into Microsoft services using an "enterprise" account.  If users are signed in to MS services using a *personal* account, they are able to access the *non*-CDP versions of both services.
