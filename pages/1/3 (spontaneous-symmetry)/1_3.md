...
Source and destination addresses of an "A to B" flow and its reflexive counterpart flow "B to A" are characterized with numeric values representing the "site-ID" and "zone-ID" of their sources and destinations.  The logic used in path selection relies on evaluating which of the "zone-ID" values (that of the source, or that of the destination is greater than the other.)

We obtain reflexive path selection creating one set of zone-ID to metric mappings for use when the destination zone-ID is higher and an "inverted" set of zone-ID to metric mappings for use when the source zone-ID is higher. 
