...
> "Integral" here is an allusion to the fact that the number of edges with labels ending with a non-zero-integer on the shortest path from any given node to a node in the top/root-level of the PAG-herarchy tell us how *deep* the given node sits in the PAG hierarchy.
